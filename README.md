...
A research repo for the [WaveKat](https://github.com/wavekat) project — interactive tools and Jupyter notebooks for working with audio models (VAD, turn detection, ASR datasets, and more).
...
Web app for testing and comparing WaveKat library backends side by side in real time. Live mic capture, WAV upload, multi-config fan-out, waveform + spectrogram + probability timelines.

Backends: webrtc-vad, silero-vad, ten-vad, firered-vad, pipecat smart-turn, livekit turn-detector. [Details →](tools/audio-lab/README.md)
...
`notebooks/` hosts Jupyter notebooks for training, validation, and dataset-splitting workflows. Python env is managed by [uv](https://docs.astral.sh/uv/).

```bash
make setup    # one-time: install all tool deps + uv sync the notebook env
