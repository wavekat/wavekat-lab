...
| [`05-pipeline-wheel.md`](05-pipeline-wheel.md) | Design for a non-interactive CLI (`wk-st run / compare / export / report`) that turns dataset → model → comparison report without clicking through notebooks. Run-output contract, cross-snapshot eval, W&B as viewer (not source of truth), 4-phase rollout. |
